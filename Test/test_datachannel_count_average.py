...
def score_fn_pytorch(pred,golden):
...
def score_fn_tensorflow(pred,golden):
    b = tf.math.argmax(input = golden)
    b1 = tf.math.argmax(input = pred)
    c = tf.keras.backend.eval(b==b1)
    score = sum(c)/len(c)
    if score == 0:
        return 0.5
    return score
...
final_score(test_count_average_pytorch, test_count_average_tensorflow)
